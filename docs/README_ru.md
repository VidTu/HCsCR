...
3. Выполните в терминале/PowerShell команду `./compileall --legacy`.
...
2. Установите Java 25 (для Gradle; для компиляции так же будут нужны
   ...
   Java toolchains) и закиньте её в `PATH` и (или) `JAVA_HOME`.
3. Выполните в терминале/PowerShell команду `./compileall`.
4. Заберите JAR-ники из папки `./build/libs/`.
...
2. Установите Java 25 (для Gradle; для компиляции так же возможно нужны 8,
   ...
   Java toolchains) и закиньте её в `PATH` и (или) `JAVA_HOME`.
3. Выполните в терминале/PowerShell команду `./compileone <версия>-<загрузчик>`.
   (например: `./compileone 1.16.5-fabric`)
...
- `ru.vidtu.hcscr.debug.asserts`: Оставлить декларации `assert` после
