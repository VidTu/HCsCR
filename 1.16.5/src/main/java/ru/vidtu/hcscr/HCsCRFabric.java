...
public final class HCsCRFabric implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        ClientPlayNetworking.registerGlobalReceiver(id, (client, handler, buf, responseSender) -> handler.getConnection().disconnect(new TranslatableComponent("hcscr.toggle.false")));
        ...
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            ...
            client.gui.setOverlayMessage(new TranslatableComponent("hcscr." + newState)
    ...
    public static boolean handleEntityHit(@NotNull Entity entity, @NotNull DamageSource source, float amount) {
        ...
        if (!HConfig.enabled || entity.removed || amount <= 0.0F || !shouldProcessEntityType(entity) ||
                !entity.level.isClientSide() || !(source.getEntity() instanceof Player) ||
