...
public class HCsCRScreen extends Screen {
    ...
    @Override
    protected void init() {
        addButton(enabled = new Checkbox(width / 2 - font.width(new TranslatableComponent("hcscr.screen.enabled")), 40, 24 + font.width(new TranslatableComponent("hcscr.screen.enabled")), 20, new TranslatableComponent("hcscr.screen.enabled"), HCsCR.enabled));
        addButton(new Button(width / 2 - 75, height - 24, 150, 20, CommonComponents.GUI_DONE, button -> minecraft.setScreen(parent)));
    }
